...
namespace ConsoleApp5
{
    internal class Program
    {
        ...
        {
            int[,] newMatrix = new int[matrix.GetLength(0), matrix.GetLength(1) + newColumns]; //новый массив нужной размерности
            for (int i = 0; i < matrix.GetLength(0); i++)
                for (int j = 0; j < matrix.GetLength(1); j++)
                    newMatrix[i, j] = matrix[i, j]; //каждый элемент старого массива переносим в новый массив
            ...
            for (int i = 0; i < matrix.GetLength(0); i++)
                for (int j = matrix.GetLength(1); j < newMatrix.GetLength(1); j++)
                {
                    ...
                    else newMatrix[i, j] = random.Next(-100, 100); //проходим по незаполненным столбцам массива и заполняем их в зависимости от способа, выбранного пользователем
                }
            return newMatrix;
        }
        ...
        {
            int[][] array = new int[strings][]; //новый массив нужной размерности
            for (int i = 0; i < strings; i++)
            {
                int columns = ReadAndCheckNumber($"Введите количество символов в {i + 1} строке массива: ", 1, 100); //новый массив нужной размерности
                array[i] = new int[columns];
                for (int j = 0; j < columns; j++)
                {
                    array[i][j] = ReadAndCheckNumber($"Введите {j + 1} элемент {i + 1} строки: ", Int32.MinValue, Int32.MaxValue); //каждый элемент вводится с клавиатуры
                }
            }
            return array;
        }
        ...
        static int[][] CreateRandomArray(int strings)
        {
            ...
            for (int i = 0; i < strings; i++)
            {
                ...
                for (int j = 0; j < columns; j++)
                {
                    array[i][j] = random.Next(-100, 100); //каждый новый элемент - случайное число
                }
            }
            return array;
        }
        ...
        /// <returns>True, если массив пустой, и false, если нет</returns>
        static bool IsArrayEmpty(int[][] array) => array == null || array.Length == 0; //лямбда-выражение, проверяющее рваный массив на пустоту аналогично предыдущим
        ...
        static void PrintArray(int[][] array)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < array.GetLength(0); i++)
                {
                    ...
                    {
                        Console.Write($"{array[i][j],6}"); //печать каждого символа в консоль с заданным форматом
                    }
                    Console.WriteLine();
                }
            }
        ...
        static int[][] DeleteRow(int[][] array, int element)
        {
            ...
            int FindElement()
            {
                int elementRow = -1; //начальное значение искомого ряда
                for (int row = 0; row < array.GetLength(0); row++)
                {
                    foreach (int item in array[row])
                    {
                        ...
                        {
                            elementRow = row; //если находим элемент - присваиваем соответствующее значение строки
                            break;
                        }
                    }
                }
            ...
            if (FindElement() == -1) //если не нашелся искомый элемент
            {
            ...
            {
                int[][] newArray = new int[array.GetLength(0) - 1][]; //новый массив нужной размерности
                for (int i = 0; i < FindElement(); i++)
                    newArray[i] = array[i]; //переносим часть старого массива в новый массив
                for (int i = FindElement(); i < newArray.GetLength(0); i++)
                    newArray[i] = array[i + 1]; //переносим оставшиеся строки изначального массива, пропустив одну
        ...
        {
            bool isExclamation = false; //изначально считаем, что восклицательных предложений нет
            foreach (char element in text)
            {
                if (IsSentencesSeparator(element)) text = text.Replace(Convert.ToString(element), Convert.ToString(element) + "*"); //прибавляем к каждому знаку конца предложэения звездочку, чтобы можно было легко разделить строку на предложения, не потеряв знак
                if (element == '!') isExclamation = true; //если нашелся "!" => true
            }
            if (isExclamation)
            {
                string[] splittedText = text.Split('*'); //делим по звездочкам на предложения
                for (int i = 0; i < splittedText.Length; i++)
                {
                    if (splittedText[i].EndsWith('!')) //если конец предложения = "!"
                    {
                        string[] words = splittedText[i].TrimEnd('!').Trim(' ').Split(' '); //убираем восклицательный знак и лишние пробелы, делим предложение на слова по пробелу
                        Array.Reverse(words); // получаем обратный порядок слов
                        splittedText[i] = String.Join(" ", words) + "!" + " "; //собираем слова обратно в приложения с "!"
                    }
                }
                string newText = String.Join("", splittedText); //собираем предложения обратно в строку
                return newText;
            }
        ...
        {
            int menuPoint; //пункт основного меню
            int matrixCreating; //выбор способа задания элементов двумерного массива
            int ragArrayCreating; //выбор способа задания элементов рваного массива
            int[,] matrix = new int[0, 0]; //двумерный массив, с которым будем работать
            int[][] ragArray = new int[0][]; //рваный массив, с которым будем работать
            string text = ""; //изначальная строка

            do
            {
                PrintMainMenu(); //вывод пунктов основного меню
                menuPoint = ReadAndCheckNumber("Выбранный вами пункт меню: ", 0, 8); //ввод пункта меню
                Console.WriteLine();
                switch (menuPoint)
                {
                    case 1:
                        PrintArrayCreatingMenu(); //вывод пунктов меню заполнения элементов
                        matrixCreating = ReadAndCheckNumber("Выбранный вами способ задания массива: ", 1, 2); //ввод пункта вспомогательного меню
                        Console.WriteLine();
                        switch (matrixCreating)
                        {
                            case 1:
                                ...
                                break; //формирование массива значениями с клавиатуры
                            ...
                            case 2:
                                ...
                                break; //формирование массива датчиком случайных чисел
                        }
                    ...
                    case 2:
                        PrintArrayCreatingMenu(); //вывод пунктов меню заполнения элементов
                        ragArrayCreating = ReadAndCheckNumber("Выбранный вами способ задания массива: ", 1, 2); //ввод пункта вспомогательного меню
                        Console.WriteLine();
                    ...
                    case 3:
                        PrintArray(matrix); //печать двумерного массива
                        PrintArray(ragArray); //печать рваного массива
                        break;
                    ...
                    case 4:
                        ...
                        {
                            int columnsNumber = ReadAndCheckNumber("Введите количество столбцов, которое вы хотите добавить: ", 0, 100); //количество новых столбцов
                            Console.WriteLine();
                            PrintFillingMehod();
                            int fillingMethod = ReadAndCheckNumber("Выбранный вами метод заполнения добавленных столбцов: ", 1, 2); //способ заполнения
                    ...
                    case 5:
                        ...
                        {
                            int findItem = ReadAndCheckNumber("Введите искомый элемент К: ", Int32.MinValue, Int32.MaxValue); //искомый элемент
                            Console.WriteLine();
                    ...
                    case 6:
                        {
                            text = ReadAndCheckString("Введите строку: "); //ввод строки
                    ...
                    case 7:
                        {
                            ...
                            {
                                Console.WriteLine("Перевернутая строка: "); //переворот восклицательных предложений
